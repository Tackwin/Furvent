...
ImGui::Interaction_Query ImGui::display(
	Tournament& tourney, const ImGui::Interaction_State& state
) noexcept {
	Interaction_Query ret;
	
	ImGui::PushID(&tourney);
	defer { ImGui::PopID(); };
	...
	auto pos = ImGui::GetCursorPos();
	auto size = ImGui::GetContentRegionAvail();
	ImVec2 PLAYER_SIZE = { 10, 10 };

	for (size_t i = 0; i < tourney.rounds.size(); ++i) {
		ImGui::PushID(i);
		defer { ImGui::PopID(); };
		
		ImVec2 p = pos + offset;
		p.y += (i + 1) * PLAYER_SIZE.y * 5;

		auto& round = tourney.rounds[i];
		for (size_t j = 0; j < round.size(); j += 3) {
			if (j > 100) break;
			ImGui::PushID(j);
			defer { ImGui::PopID(); };
			...
			if (j + 1 >= round.size()) break;
			hovered = false;
			clicked = false;
			clicked |= ImGui::ButtonBehavior(
				{ p - PLAYER_SIZE / 2, p + PLAYER_SIZE / 2 },
				window->GetID((const void*)(j + 1)),
				&hovered,
				nullptr,
				ImGuiButtonFlags_MouseButtonLeft
			);
			...
			if (j + 2 >= round.size()) break;
			hovered = false;
			clicked = false;
			clicked |= ImGui::ButtonBehavior(
				{ p - PLAYER_SIZE / 2, p + PLAYER_SIZE / 2 },
				window->GetID((const void*)(j + 2)),
				&hovered,
				nullptr,
				ImGuiButtonFlags_MouseButtonLeft
			);
...
void ImGui::display(Table_Agent& agent) noexcept {
	ImGui::PushID(&agent);
	defer { ImGui::PopID(); };

	ImGui::Begin("Table Agent");
	defer { ImGui::End(); };
